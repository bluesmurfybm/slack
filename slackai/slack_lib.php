...
/**
 * curl SSL 옵션. CA 번들이 있으면 인증서 검증을 켠다(php.ini curl.cainfo/openssl.cafile 또는 slackai/vendor/cacert.pem — https://curl.se/ca/cacert.pem).
 * WAMP 기본 PHP 는 CA 번들이 없어 검증을 켜면 모든 HTTPS 가 실패하므로, 번들이 없을 때만 끈다(slack/ 은 항상 꺼져 있음).
 */
function slackCurlSslOpts() {
    static $opts = null;
    if ($opts !== null) return $opts;
    $ca = ini_get('curl.cainfo') ?: (ini_get('openssl.cafile') ?: '');
    if ($ca === '' && is_file(__DIR__ . '/vendor/cacert.pem')) $ca = __DIR__ . '/vendor/cacert.pem';
    $opts = ($ca !== '' && is_file($ca)) ? [CURLOPT_SSL_VERIFYPEER => true, CURLOPT_CAINFO => $ca]
...
function slackGet($method, $token, $params) {
    ...
    for ($try = 0; $try < 4; $try++) {
        ...
        curl_setopt_array($ch, slackCurlSslOpts());   // CA 번들 있으면 검증 켬, 없으면 끔
        $body = curl_exec($ch);
...
function slackUploadFile($token, $channel, array $file, $comment = '', $threadTs = null) {
    ...
    curl_setopt_array($c, slackCurlSslOpts());   // CA 번들 있으면 검증 켬, 없으면 끔
    curl_exec($c);
...
function slackFetchRows($token, $listId, $sinceUpdated = 0, $colMap = null) {
    ...
    return slackFetchRowsFromItems($token, $listId, $data['items'], $sinceUpdated, $colMap);
}

/**
 * 이미 받아 둔 항목 배열(slackLists.items.list 의 items 또는 items.info 의 record 1건)을 정규화한다.
 *  slackai: sync.php 의 단건(item=) 모드가 items.info 결과 1건으로 같은 정규화를 타기 위해 분리했다.
 * @param array $items Slack 항목 배열(각각 id/fields/date_created/updated_timestamp)
 */
function slackFetchRowsFromItems($token, $listId, array $items, $sinceUpdated = 0, $colMap = null) {
