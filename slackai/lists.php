...
<div class="wrap">
  <div class="head">
    <h1>WorkHub AI <span class="badge" id="count"></span></h1>
    ...
    <div class="filters iconbar">
      ...
      <div class="dropdown" id="analyzeDd">
        ...
        <div class="ddmenu">
          <a href="tags/tags.php">🏷️ 태그 관리</a>
          <a href="repos/repos.php">📁 레포 매핑</a>
          <a href="ai/jobs.php">🤖 AI 작업 로그</a>
          <a href="ai/lessons.php">📚 학습 노트</a>
          <a href="ai/settings.php">⚙️ AI 설정</a>
          <a href="similar/similar.php">🔍 유사 이력</a>
        </div>
      </div>
...
<?php include __DIR__ . '/ai/panel.php'; ?>
...
let showAi = true;                        // 상세에서 🤖 AI 패널(하단 전폭) 표시 여부(기본 표시, 🤖 AI 버튼으로 토글)
    ...
    <div class="detail${showCmts?' with-cmts':''}">
      <div class="detail-main">
        ...
        <div class="links">
          ...
          <button type="button" class="aiToggle ${showAi?'on':''}" title="AI 요약·유사 사례·플랜·실행·커밋·검토 패널">🤖 AI${showAi?' 닫기':''}</button>
...
const RECENT_KEY = "slackai_recent_reacts_" + (ME_ID || "me");
...
function bindRows(box){
  box.querySelectorAll(".row").forEach(el=>{
    el.addEventListener("click",()=>{
      ...
      if(opening && showAi) AI.load(id);          // 열 때 🤖 AI 패널 상태 로드
    });
  });
  box.querySelectorAll(".aiToggle").forEach(el=>{
    el.addEventListener("click", e=>{
      e.stopPropagation();
      showAi = !showAi;
      saveView();
      render();
      if(showAi && openId) AI.load(openId);
    });
  });
  ...
  bindLightbox(box);
  if(window.AI) AI.bind(box);   // 🤖 AI 패널: 캐시된 상태 다시 그리기 / 이벤트 바인딩(없으면 로드)
}
...
const CMT_W_KEY="slackai_cmt_w", CMT_H_KEY="slackai_cmt_h", CMT_IN_H_KEY="slackai_cmt_input_h";
...
function render(){
  ...
  document.title = (uc>0 ? `(${uc}) ` : "") + "유지보수 요청 · AI";
...
const FILTER_KEY = "slackai_filters";   // 운영 lists.php 와 별도 키
...
const PRESET_KEY = "slackai_filter_presets_" + (ME_ID || "me");   // 로컬 캐시(빠른 표시용). 실제 저장소는 DB(prefs.php)
...
const PRESET_SEL_KEY = "slackai_preset_sel_" + (ME_ID || "me");
...
const HDRBTN_KEY = "slackai_hdrbtns_" + (ME_ID || "me");
const HEADER_BTNS = [
  {id:"dueBtn",       label:"지연"},
...
const UIPREF_KEY = "slackai_uipref_" + (ME_ID || "me");
...
function refreshTick(){
  // slackai: 주기 sync 호출 없음 — Slack 이벤트를 워커가 받아 DB 를 갱신하고, 화면은 status.php 변화만 본다.
...
const VIEW_KEY = "slackai_view";
function saveView(){ localStorage.setItem(VIEW_KEY, JSON.stringify({ splitOn, showHidden, showCmts, showAi, archivedMode, archPageSize })); }
function restoreView(){
  ...
  if(typeof v.showAi === "boolean")     showAi = v.showAi;       // 🤖 AI 패널 열기/닫기 상태 복원
...
/* ---------- 최근 본 항목 ---------- */
const RECENTV_KEY = "slackai_recentview_" + (ME_ID || "me");
...
async function pollStatus(){
  try{
    ...
    // AI 워커 상태 표시등: heartbeat 90초 내 + Slack 이벤트(Socket Mode) 연결 = 초록 / 워커만 살아있음 = 노랑 / 응답 없음 = 빨강
    const dot = document.getElementById("daemonDot");
    if(dot){
      const wAge = Math.round(Date.now()/1000 - (s.ai_worker_at||0));
      const alive = (s.ai_worker_at||0) > 0 && wAge < 90;
      const push  = !!s.ai_push_connected;
      dot.style.color = (alive && push) ? "#1e8e3e" : (alive ? "#f9ab00" : "#d93025");
      dot.dataset.tip = (alive && push) ? `AI 워커 정상 · Slack 이벤트 수신 중 (heartbeat ${wAge}초 전, 열린 작업 ${s.ai_active||0}건)`
                      : alive ? `AI 워커 동작 중이나 Slack 이벤트 연결 없음 (${wAge}초 전) — SLACK_APP_TOKEN/워크플로 확인`
                      : `AI 워커 응답 없음 — slackai/worker/start_worker.bat 확인`;
    }
    if(window.AI && typeof AI.onStatus === "function") AI.onStatus(s);   // AI 패널: ai_changed_at 변화 시 열린 패널만 갱신
...
/* ---------- 진입 시 1회 증분 동기화 ----------
   slackai 는 주기 폴링을 하지 않는다: Slack 이벤트(워크플로 커스텀 스텝 → 워커)가 DB 를 갱신한다.
   페이지를 열 때 한 번만 증분 동기화해 워커가 내려가 있던 동안의 변경을 따라잡는다.
   마지막 동기화가 5분 이내면 그것도 건너뛴다(여러 사람이 동시에 열어도 Slack 호출 최소화). */
async function bgSync(){
  if (lastSyncedAt && (Date.now() / 1000 - lastSyncedAt) < 300) return;
