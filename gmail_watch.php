...
// 중복 실행 방지 (여러 인스턴스가 뜨면 연결·로그 낭비)
$__lock = fopen(__DIR__ . '/.gmail_watch.lock', 'c');
if (!$__lock || !flock($__lock, LOCK_EX | LOCK_NB)) exit("이미 실행 중 — 종료\n");
...
while (true) {                                             // 끊기면 자동 재접속
    ...
    try {
        ...
        wlog('연결됨 — IDLE 대기');
        gmeta_set('gmail_watch_beat', (string)time());

        // 죽은 소켓 감지용 쓰기 헬퍼: TLS 가 끊기면 fwrite 가 false/0 → 즉시 재접속으로
        // (이걸 안 하면 끊긴 소켓에 IDLE/DONE 을 무한 반복 → CPU 점유 + 경고 로그 폭주)
        $send = function ($data) use ($fp) {
            $n = @fwrite($fp, $data);
            if ($n === false || $n === 0) throw new RuntimeException('소켓 쓰기 실패(연결 끊김)');
        };
        while (true) {
            ...
            while (true) {
                $l = fgets($fp, 8192);
                if ($l === false) {
                    if (feof($fp)) throw new RuntimeException('연결 끊김(EOF)');
                    $meta = stream_get_meta_data($fp);
                    if (!empty($meta['timed_out'])) break; // 진짜 타임아웃 틱 → 재-IDLE (연결 유지용)
            ...
            // 스핀 안전장치: 대기 없이 1초 미만에 사이클이 도는 건 비정상 → 재접속
            if (!$changed && (microtime(true) - $cycleStart) < 1.0) {
                throw new RuntimeException('IDLE 사이클 비정상(즉시 반복) — 재접속');
            }
            gmeta_set('gmail_watch_beat', (string)time()); // 생존 신호 (상태 표시등용)
            if ($changed) {
                wlog('변경 푸시 감지 → 동기화');
                $r = gmail_sync_pass();
