...
// 도메인 스크립트는 FastAPI 시절의 절대경로(/learningapi/...)를 그대로 쓴다. 이 모듈은
// 포털 하위 폴더(/learn/)에서 도므로 경로를 프런트 컨트롤러의 상대주소로 옮겨 준다 —
// 호출부를 전부 고치는 것보다 갈아탈 자리가 한 군데인 편이 낫다.
function learnApiURL(path) {
  if (!path.startsWith("/learningapi/")) return path;
  const [p, qs] = path.slice("/learningapi".length).split("?");
  return "api.php?p=" + encodeURIComponent(p) + (qs ? "&" + qs : "");
}

const certDownloadURL = (rid, cid) =>
  learnApiURL(`/learningapi/requests/${rid}/certs/${cid}/download`);

async function api(path, opts) {
  const r = await fetch(learnApiURL(path),
