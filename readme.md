...
카드로 감싸면 그 칸만 무겁게 튀어서 **내용만 남겼다.** 공지 줄과 같은 높이·같은
생김새라 두 칸이 한 덩어리로 읽힌다.

```
D-12   🌴 추석 연휴                      09.30(수)
D-28   🎈 전사 워크숍                    10.16(금) · 속초
```

구분은 **왼쪽 D-day 글자색(`--k-ink`)과 그림** 둘뿐이다. 바탕까지 칠하면 줄
여덟 개가 알록달록해져 오히려 안 읽힌다. 임박도는 색을 바꾸지 않고 굵기만
건드린다 — 오늘 것은 제목이 굵어지고 D-day 가 맥박친다.

날짜는 올해면 연도를 뗀다(`09.30(수)`). 거의 다 올해라 네 글자가 자리만 차지한다.
...
화면 쪽은 `.ev-row.k-<key>{--k-ink:…}` 한 줄이면 된다.
