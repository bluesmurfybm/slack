...
- **켜진 점은 '밝은 발광색'(`--k-lit`)** 을 따로 쓴다. 종류 색을 그대로 쓰면
  회의(`#1C5DE5`)·조사(`#5B6577`) 같은 어두운 색이 짙은 바탕에서 대비 3.5 밖에
  안 나와 글자가 뭉개진다. 밝게 올린 값이라 어느 종류인지는 그대로 구분된다
- **꺼진 점은 무슨 일정이든 같은 색**(`#1C2433`). 넘어갈 때 글자만 갈아입는다
- 한글은 못 찍으므로 `진행중` 은 `NOW` 로 옮긴다(`pixText`)

| 종류 | 본래 색 | 발광색 | 바탕 대비 |
|---|---|---|---|
| 회의 | `#1C5DE5` | `#79B0FF` | 3.5 → 7.9 |
| 조사 | `#5B6577` | `#B3BECF` | 3.3 → 8.5 |
| 마감 | `#D9820B` | `#FFB43F` | 6.6 → 8.9 |

아홉 종류 모두 발광색으로 바꾸면 대비가 **6.8~8.9** 가 된다.
