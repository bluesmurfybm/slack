...
- 판은 **21×11 = 231점 붙박이**. `D-3` 이든 `D-128` 이든 크기도 점 개수도 같다
- 글자는 3×5 픽셀 글자(`PIX_FONT`)로 찍고, **점 칸 단위로 가운데를 맞춘다** —
  글자 수로 맞추면 네 글자일 때 한쪽으로 한 칸 치우친다
- 글자 둘레(좌우 1줄, 위아래 3줄)에도 꺼진 점을 깔아 테두리 없이 네모가 된다
- **켜진 점만 종류 색**(`--k-ink`), **꺼진 점은 무슨 일정이든 같은 회색**(`#D7DFEC`).
  넘어갈 때 글자만 갈아입고 판은 그대로 있는 것처럼 보인다
