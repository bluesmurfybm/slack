...
**배경** — WMO 날씨 코드를 아홉 갈래(맑음·구름조금·흐림·안개·이슬비·비·눈·뇌우·밤)로
묶어 `body[data-wx]` 에 싣고, **화면 위쪽만** 옅게 물들인 뒤 아래로 가면서 본래
색으로 돌아온다. 통째로 칠하면 글 읽기가 나빠지고, 위만 물들이면 "오늘 날씨"
라는 느낌만 남는다.
...
- 오늘 일정은 D-day 점이 아주 천천히 숨 쉰다(`pixBreathe`)
...
**카드로 감싸지 않는다.** 바탕을 깔면 그 칸만 무겁게 튀어 화면의 시선을 혼자
가져간다. 힘은 **왼쪽 달력 한 장**에만 주고 나머지는 글자로 둔다.
...
달력처럼 보이게 하는 건 두 가지다 — **위 테두리만 굵게**(5px) 하고, **고리 두 개**를
그 위에 걸친다. 벽에 걸린 달력 한 장처럼 읽힌다.

**달력 안쪽은 통째로 픽셀 판이다.** `D-43` 이 글자 하나까지 전부 점으로 찍힌다
(3×5 픽셀 글자, `PIX_FONT`).

```
D-3     ·██·  ···  ···  ███  ···     ← 다섯 글자 자리가 붙박이
D-43    ██·  ···  █·█  ███  ···         판 크기도 점 개수도 늘 같다
```

핵심은 **판이 고정되어 보이는 것**이다.

- 자리는 **다섯 글자로 붙박이**(`PIX_CELLS`). `D-3` 이든 `D-128` 이든 판 크기와
  점 개수(19×5=95)가 같고, 짧으면 가운데로 놓인다
- 글자 사이 빈 칸에도 꺼진 점이 남는다 — 그래야 '판' 으로 보인다
- **켜진 점만 종류 색**(`--k-ink`)을 입고, **꺼진 점은 무슨 일정이든 같은 회색**이다.
  넘어갈 때 글자만 색이 바뀌고 판은 그대로 있는 것처럼 보인다
- 한글은 못 찍으므로 `진행중` 은 `NOW` 로 옮긴다(`pixText`)

> 한때 검은 판에 네온 7세그먼트를 얹어 봤는데 **포털의 다른 화면과 겉돌았다.**
> 바탕은 그대로 두고 점만 종류 색으로 찍으니 결이 맞는다.

| | |
|---|---|
| 달력 틀 | 92×48 붙박이. 테두리와 켜진 점이 종류 색(`--k-ink`) |
| 제목 | 15px 굵게 + 종류 그림 |
| 날짜 \| 장소 | 12px, 흐리게 |

오늘 것은 점이 2.4초에 걸쳐 아주 천천히 숨 쉰다. 깜빡이면 업무 화면에서 성가시다.
...
화면 쪽은 `.ev-item.k-<key>{--k-ink:…}` 한 줄이면 된다.
