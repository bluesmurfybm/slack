...
**카드도 테두리도 없다.** 왼쪽은 점으로만 이루어진 네모(픽셀 판)고, 오른쪽에
제목과 날짜가 붙는다.
...
- **꺼진 점은 연한 네이비**(`#C9D2E2`), 무슨 일정이든 같은 색이다. 처음엔 검정에
  가까웠는데(`#1C2433`) 판이 화면 한복판의 검은 구멍처럼 튀었다. 밝게 올리니
  켜진 점이 '그 위에 찍힌 글자' 로 읽힌다. 카드 흰 바탕과는 1.52 라 판의
  네모꼴도 그대로 보인다
- **켜진 점은 종류 색을 한 단 내린 값**(`--k-ink`). 바탕을 밝혔으니 글자는 거꾸로
  진해야 한다. 본래 색을 그대로 쓰면 마감(`#D9820B`)·행사(`#E2622A`)·휴무
  (`#0F9B8E`) 가 대비 2 언저리라 글자가 바탕에 녹는다
- 한글은 못 찍으므로 `진행중` 은 `NOW` 로 옮긴다(`pixText`)

| 종류 | 본래 색 | 판 글자색 | 바탕(`#C9D2E2`) 대비 |
|---|---|---|---|
| 조사 | `#5B6577` | `#384152` | 6.7 |
| 경사 | `#D6336C` | `#961643` | 5.5 |
| 휴무 | `#0F9B8E` | `#0A5F57` | 5.0 |
| 마감 | `#D9820B` | `#7E4A05` | 4.8 |
| 교육 | `#7B4DD8` | `#5227A8` | 6.2 |
| 작업 | `#4A6B8A` | `#2D445D` | 6.6 |
| 행사 | `#E2622A` | `#8E390F` | 5.0 |
| 회의 | `#1C5DE5` | `#12409E` | 6.1 |
| 그 밖 | `#5A73A8` | `#36496F` | 5.9 |

아홉 종류 모두 **4.8 이상** — WCAG AA(4.5) 를 넘는다. 본래 색은 관리 화면의
`.dday` 알약이 그대로 쓰므로 두 곳의 색이 어긋나 보이지 않게 계열을 맞췄다.
...
새로 만들 때만 `styles/board.css` 에 `.ev-pix.k-<key>{--k-ink:…}` 한 줄을 더한다.
