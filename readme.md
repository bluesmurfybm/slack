...
- 판은 **21×7 = 147점 붙박이**. `D-3` 이든 `D-128` 이든 크기도 점 개수도 같다
- 글자는 3×5 픽셀 글자(`PIX_FONT`)로 찍고, **점 칸 단위로 가운데를 맞춘다** —
  글자 수로 맞추면 네 글자일 때 한쪽으로 한 칸 치우친다
- 글자 둘레(좌우·위아래 한 줄씩)에도 꺼진 점을 깔아 테두리 없이 네모가 된다.
  여백을 한 줄로 줄여 **글자가 판 높이의 70% 남짓**을 차지한다
- **켜진 점만 종류 색**(`--k-ink`), **꺼진 점은 무슨 일정이든 검정**(`#0A0D14`).
  아홉 종류 색 모두 검정 위에서 대비 3.5 이상이라 어떤 일정이든 읽힌다
