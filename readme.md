...
넘기는 방식은 칸마다 다르다. `makeSlider(..., mode)` 의 네 번째 인자로 고른다.
...
한 번에 한 장만 보이는 칸이라 미는 것보다 넘기는 쪽이 맞다.

```
.flip-win    perspective 를 가진 창
  └ .slide-track   장들이 position:absolute 로 겹쳐 쌓인다(맨 앞이 z-index 최상)
       다음 → 지금 장이 위쪽을 경첩으로 젖혀지며(rotateX -96°) 사라지고,
              아래 있던 장이 그대로 드러난다
       이전 → 넘어올 장을 맨 위로 올려 젖혀진 채 두었다가 내려와 덮는다
```

`transform-origin: top center` 가 경첩이고, 젖혀질수록 `brightness` 가 떨어져
그늘이 진다. **움직이는 건 언제나 한 장뿐이라** 장이 많아도 가볍다.

목록을 복제하지 않아 줄이 늘어도 DOM 이 두 배가 되지 않는다.

**손으로 넘기기** — 머리말의 `⌃ 3 / 8 ⌄` 로 바로 앞뒤로 옮긴다. 건수가 많으면
...
- 카드와 줄이 한 박자씩 밀려 올라온다(`evRise`, 50ms 간격)
- 배경 원이 9초에 걸쳐 아주 천천히 돈다(`evDrift`)
- hover 하면 빛이 한 번 쓸고 지나가고(`evShine`), 줄은 오른쪽으로 3px 밀리며
  D-day 알약이 살짝 커진다
- 오늘이면 숫자가 맥박치고(`ddayPulse`), 일주일 안이면 아주 약하게 숨 쉰다(`ddayBreathe`)
...
### 일정 카드의 색 — 종류가 색, 임박도가 진하기

진한 단색 카드는 화면의 시선을 혼자 다 가져간다. 옅은 바탕에 글자로 읽히게 두고,
힘은 **왼쪽 띠와 D-day 숫자**에만 준다.

두 가지를 한 색에 섞으면 둘 다 안 읽히므로 축을 나눴다.

| 축 | 무엇이 정하나 | 어떻게 보이나 |
|---|---|---|
| **색** | 일정 종류 (`--k-ink`) | 왼쪽 띠 · D-day 숫자 · 바탕 |
| **진하기** | 임박도 (`heat`) | `later` 8% → `soon` 13% → `today` 18%, 띠도 3→4→5px |
...
축하 색이 붙지 않는다. 낱말을 늘리려면 해당 줄에 덧붙이기만 하면 되고, 화면 쪽은
`--k-ink` 한 줄만 있으면 된다.

바탕·테두리는 `color-mix()` 로 `--k-ink` 하나에서 만들어 낸다. 지원하지 않는
브라우저를 위해 바로 앞줄에 고정값을 같이 적어 뒀다.
