...
**D-day 숫자는 점으로 찍는다**(3×5 픽셀 글자, `PIX_FONT`). 숫자 하나가 열다섯
칸이고, 켜진 점은 종류 색, **꺼진 점도 같은 색을 14% 로 남겨** '점판' 이라는 결이
보이게 했다. 자릿수가 늘면 달력 틀이 알아서 넓어진다(`min-width` + `padding`).

> 한때 검은 판에 네온 7세그먼트를 얹어 봤는데 **포털의 다른 화면과 겉돌았다.**
> 바탕은 그대로 두고 점만 종류 색으로 찍으니 결이 맞는다. `D-DAY`·`진행중` 처럼
> 숫자가 아닌 것은 점 대신 글자로 둔다 — 억지로 점으로 만들면 읽기만 나빠진다.

| | |
|---|---|
| 달력 틀 | 높이 54, 테두리·`D-`·점 모두 종류 색(`--k-ink`) |
| 제목 | 15px 굵게 + 종류 그림 |
| 날짜 \| 장소 | 12px, 흐리게 |
