...
일정 칸만은 머리말 줄이 없는 **위젯**이다. 내용이 칸을 꽉 채우고 위아래 화살표로
한 건씩 넘긴다.
...
**전광판은 SVG 로 직접 그린다.** 바깥 글꼴을 받아 오지 않으려는 것이고,
꺼진 칸도 아주 흐리게 남겨 진짜 전광판처럼 보이게 했다(`SEG_ON`, `SEG_PTS`).

- `D-43` 처럼 숫자면 `D-` 를 앞에 두고 숫자만 전광판에 올린다
- `D-DAY` · `진행중` · `D+3` 은 숫자가 아니라서 같은 판에 빛나는 글씨로 얹는다
- 오늘 것은 전광판이 천천히 깜빡인다

바탕은 일정 종류 색(`--k-ink`)을 14% 로 깔아, 무슨 일인지 색으로 먼저 읽히게 했다.
제목과 날짜 사이의 가는 선은 **전광판 쪽에 준다** — 제목에 붙이면 제목의
`overflow:hidden` 에 잘려 안 보인다.
