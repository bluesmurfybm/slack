...
function dti_route_materials(array $ctx, array $req, int $tid): Response {
...
function dti_material_attach_link(array $ctx, int $tid, string $slot, array $body): Response {
...
function dti_material_attach_file(array $ctx, int $tid, string $slot, array $files): Response {
    [$topic, $pres] = dti_material_guard($ctx, $tid);

    $file = $files['file'] ?? null;
    if (!is_array($file)) {
        // post_max_size 를 넘기면 PHP 가 $_FILES 를 통째로 비워 보낸다. 그것도 용량 초과다
        throw new ApiException($ctx['config']['max_upload_mb'] . 'MB 까지 올릴 수 있습니다', 413);
    }
...
function dti_material_detach(array $ctx, int $tid, string $slot): Response {
    [$topic, $pres] = dti_material_guard($ctx, $tid);
...
function dti_material_download(array $ctx, int $tid, string $slot): Response {
    $pdo = $ctx['pdo'];
...
function dti_material_save(array $ctx, array $topic, array $holder, string $slot): Response {
    $pdo = $ctx['pdo'];
    ...
    return Response::json(dti_topic_present($topic, dti_presentation_of_topic($pdo, (int)$topic['id'])));
}
